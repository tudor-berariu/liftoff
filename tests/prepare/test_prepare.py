...
class TestPrepareCLI:
    ...
    def _check_subdirectory_validity(self, subdir_path):
        ...
        assert len(subfolders) == 1, f"Expected 1 subfolder in {subdir_path}, found {len(subfolders)}"
        assert cfg_hash_file in contents, f"Missing {cfg_hash_file} file in {subdir_path}"
        ...
        for expected_file in expected_files:
            assert expected_file in subfolder_contents, f"Missing {expected_file} in {subfolder_path}"
    ...
    def test_prepare_gridsearch_config(self, method_scoped_directory):
        ...
        config_folder_path = os.path.join(
            shared_resources_location, config_directory
        )
        ...
        subdirs = [d for d in os.listdir(method_scoped_directory) 
        ...
        assert date_pattern.match(subdir_name), "Folder name does not match the expected date-time format"
        ...
        assert len(sub_subdirs) == 2, f"Expected 2 subdirectories in {subdir_name}, found {len(sub_subdirs)}"
