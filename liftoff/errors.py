...
from .version import welcome_msg
...
def main():
    ...
    title = urwid.Text(
        ("title", welcome_msg(color=False)), align="center"
    )
    ...
    stats = urwid.Text(
        f"Error {(crt_err_idx + 1):d}/{nerrors:d} | "
    ...
    details_text = urwid.Text(
        config_to_string(dict_to_namespace(crt_details), color=False)
    )
    pile = urwid.Pile(
        [
            title_map,
